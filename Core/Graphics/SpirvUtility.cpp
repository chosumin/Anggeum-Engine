    ...
    const string& shaderPath, const vector<uint32_t>& spirvBinary)
{
    ...
    {
        unsigned set = compiler.get_decoration(uniform_buffer.id, spv::DecorationDescriptorSet);
        unsigned binding = compiler.get_decoration(uniform_buffer.id, spv::DecorationBinding);
        const spirv_cross::SPIRType& type = compiler.get_type(uniform_buffer.base_type_id);
